...
#include <zcm/detail/vec3_access_int.hpp>
...
namespace zcm
{
    ...
    struct ivec3
    {
        ...
        union{
            int32_t _data[3];
            detail::scalar_accessor_3<int32_t, 0> x, r, s;
            detail::scalar_accessor_3<int32_t, 1> y, g, t;
            detail::scalar_accessor_3<int32_t, 2> z, b, p;
    ...
    bool operator==(const ivec3& first, const ivec3& second) noexcept;
    bool operator!=(const ivec3& first, const ivec3& second) noexcept;

    ivec3 operator -(ivec3 first) noexcept;
    ivec3 operator +(ivec3 first) noexcept;

    ivec3 operator +(const ivec3& first, const ivec3& second) noexcept;
    ivec3 operator -(const ivec3& first, const ivec3& second) noexcept;
    ivec3 operator *(const ivec3& first, const ivec3& second) noexcept;
    ivec3 operator /(const ivec3& first, const ivec3& second) noexcept;

    ivec3 operator *(const ivec3& vec, const int32_t scalar) noexcept;
    ivec3 operator /(const ivec3& vec, const int32_t scalar) noexcept;

    ivec3 operator *(const int32_t scalar, const ivec3 vec) noexcept;
    ivec3 operator /(const int32_t scalar, const ivec3 vec) noexcept;
}
